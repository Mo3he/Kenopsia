...
## Crossfade Breaks After a Route Change ✅ Fixed in 1.2
...
## Open Decisions and Chores (noted 2026-09-22)

Parked deliberately, not forgotten. Nothing here blocks 1.2, which is in App
Store review at version 1.2 (build 3).

### Monetisation: undecided

A user offered to pay for an "upgrade version". No decision taken, and copy
should stay neutral until one is.

Two places currently lean free-forever and would need revisiting if a paid tier
ever appears:

| Where | Line |
|---|---|
| App Store description | "No accounts, no tracking, no ads" and the "No subscription to play your own music" phrasing |
| Website hero and support section | "Free · No ads · No accounts · No tracking", and "Kenopsia is free" |

The website's ask is donations only (Buy Me A Coffee, GitHub Sponsors), which
stays true either way. The App Store copy is the part that could box things in.

### Chores

| Chore | Detail |
|---|---|
| Bump `xcodeVersion` in `project.yml` | Still stamps Xcode 16, so `LastUpgradeCheck = 1600`. Causes Xcode's "enable dead code stripping" prompt (already enabled everywhere, Debug and Release, so the prompt is a false positive) and the endless `.xcscheme` churn where xcodegen writes version 1.7 and Xcode rewrites 1.3. Metadata only; does not affect the build. Do NOT accept Xcode's "Update to recommended settings", which edits the generated project. |
| Point `origin` at the new URL | `git remote set-url origin https://github.com/Mo3he/Kenopsia.git`. The repo moved from `Loudmouth.git`; pushes work by redirect today, which is not guaranteed to last. |
| Strip em dashes from store copy | The What's New notes, the description edits and the drafted support email were written with em dashes, which the user does not want. The promotional text is already clean. |
...
## Library List / Grid Toggle ✅ Done in 1.2
...
## Duplicate Song Detection ✅ Done in 1.2
...
### Enhancements ✅ Mostly done in 1.2

Requested by a user (Roy, Sep 2026): "a more complete CarPlay interface."

Closed in 1.2:

- Genres browsing, backed by the shared `GenreIndex`
- Shuffle All at the top of the Library tab
- Truncated lists now end with a "N more not shown" row instead of silently dropping entries
- Up Next / Album-Artist buttons gated on real queue state

Not a gap after all: the header comment claimed four tabs, but `CPSearchTemplate`
cannot be a tab and Now Playing is system-managed, so two tabs is correct by
design. Comment corrected.

Still open:

- The 1.2 CarPlay work has never been seen rendered. The CarPlay display cannot be activated headlessly (`simctl` lists the screen but reports no surfaces, and Device Hub is not scriptable the way Simulator's menus were), so it needs a manual pass in Device Hub. Given CarPlay's history of killing the scene on API misuse, treat this as required rather than optional.
